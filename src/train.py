...
def parseArgs():
    ...
    parser.add_argument('--lr_crit', action="store", type=float, dest="lr_crit", default=0.001)
    parser.add_argument('--lr_act', action="store", type=float, dest="lr_act", default=0.0001)
    parser.add_argument('--batch_size', action="store", type=int, dest="batch_size", default=64)
    parser.add_argument('--buffer_size', action="store", type=int, dest="buffer_size", default=1e5)
    parser.add_argument('--critic_l2_reg', action="store", type=float, dest="critic_l2_reg", default=1e-2)
    parser.add_argument('--num_rollouts', action="store", type=int, dest="num_rollouts", default=100)
    parser.add_argument('--num_epochs', action="store", type=int, dest="num_epochs", default=100)
    parser.add_argument('--num_cycles', action="store", type=int, dest="num_cycles", default=20)
    parser.add_argument('--train_steps', action="store", type=int, dest="train_steps", default=50)
    parser.add_argument('--stddev', action="store", type=float, dest="stddev", default=0.2)
    parser.add_argument('--hidden_size', action="store", type=int, dest="hidden_size", default=64)
    parser.add_argument('-version', action="store", dest="version", default='')
